<script setup lang="ts">
import type { EditorialTwoColumnSection } from "~~/types/content"

const props = defineProps<{
  section: EditorialTwoColumnSection
}>()

function hasYoutube(column: EditorialTwoColumnSection["left"]): column is Extract<EditorialTwoColumnSection["left"], { youtube: { url: string } }> {
  return typeof column === "object" && column !== null && "youtube" in column && typeof column.youtube?.url === "string"
}
</script>

<template>
  <section class="border p-8 md:p-10">
    <p v-if="section.eyebrow" class="text-sm uppercase">
      {{ section.eyebrow }}
    </p>

    <div class="mt-4 grid gap-8 md:grid-cols-2 md:gap-10">
      <div>
        <h2 v-if="section.left.title" class="text-2xl">
          {{ section.left.title }}
        </h2>
        <YouTubeEmbed
          v-if="hasYoutube(section.left)"
          :url="section.left.youtube.url"
          :title="section.left.youtube.title"
          :show-heading="false"
          :class="section.left.title ? 'mt-4' : ''"
        />
        <div v-else :class="section.left.title ? 'mt-4 space-y-4 text-base leading-7' : 'space-y-4 text-base leading-7'">
          <p v-for="paragraph in section.left.paragraphs" :key="paragraph">
            {{ paragraph }}
          </p>
        </div>
      </div>

      <div>
        <h2 v-if="section.right.title" class="text-2xl">
          {{ section.right.title }}
        </h2>
        <YouTubeEmbed
          v-if="hasYoutube(section.right)"
          :url="section.right.youtube.url"
          :title="section.right.youtube.title"
          :show-heading="false"
          :class="section.right.title ? 'mt-4' : ''"
        />
        <div v-else :class="section.right.title ? 'mt-4 space-y-4 text-base leading-7' : 'space-y-4 text-base leading-7'">
          <p v-for="paragraph in section.right.paragraphs" :key="paragraph">
            {{ paragraph }}
          </p>
        </div>
      </div>
    </div>
  </section>
</template>

...
const featurePositionClasses: Record<NonNullable<EditorialFeatureImageSection["textPosition"]>, string> = {
  ...
  "bottom-left": "bottom-8 left-2 md:bottom-8 md:left-8",
  "bottom-right": "bottom-8 right-2 md:bottom-8 md:right-8",
}
...
<template>
  <section class="relative overflow-hidden">
    ...
    <div
      class="absolute max-w-72 md:max-w-sm bg-jw_red p-3 text-jw_blue md:p-6"
      :class="getFeatureTextPosition(props.section)"
    >
      <p v-if="props.section.overline" class="text-xs uppercase">
        {{ props.section.overline }}
      </p>
      <h2 v-if="props.section.title"  class="mt-2 text-md md:text-2xl">
        {{ props.section.title }}
      </h2>
    ...
    <ImageCredit :credit="props.section.image.photographerCredit || props.section.credit" />
  </section>
</template>
